 ...
 */
package studentcapture.ltistarter.lti;

import studentcapture.ltistarter.model.*;
import studentcapture.ltistarter.repository.AllRepositories;
...
public class LTIDataService {
    ...
    public boolean loadLTIDataFromDB(LTIRequest lti) {
        ...
        q.setMaxResults(1);
        q.setParameter("key", BaseEntity.makeSHA256(lti.getLtiConsumerKey()));
        ...
        if (includesService) {
            q.setParameter("service", BaseEntity.makeSHA256(lti.getLtiServiceId()));
        }
        if (includesSourcedid) {
        ...
        if (rows == null || rows.isEmpty()) {
            log.info("LTIload: No lti.results found for key=" + lti.getLtiConsumerKey());
        } else {
            // k, c, l, m, u, s, r
            ...
            lti.loaded = true;
            log.info("LTIload: loaded data for key=" + lti.getLtiConsumerKey() + " and context=" + lti.ltiContextId + ", complete=" + lti.complete);
        }
        return lti.loaded;
    }
    ...
    public int updateLTIDataInDB(LTIRequest lti) {
        ...
        if (lti.service == null && lti.getLtiServiceId() != null && lti.ltiSourcedid != null) {
            LtiServiceEntity newService = new LtiServiceEntity(lti.getLtiServiceId(), lti.key, null);
            lti.service = repos.services.save(newService);
            inserts++;
            serviceCreated = true;
            log.info("LTIupdate: Inserted service id=" + lti.getLtiServiceId());
        } else if (lti.service != null) {
            repos.entityManager.merge(lti.service); // reconnect object for this transaction
            lti.setServiceId(lti.getService().getServiceKey());
            log.info("LTIupdate: Reconnected existing service id=" + lti.getLtiServiceId());
        }

        // If we just created a new service entry but we already had a result entry, update it
        if (serviceCreated && lti.result != null && lti.getLtiServiceId() != null && lti.ltiSourcedid != null) {
        ...
        if (lti.result == null
                ...
                && lti.getLtiServiceId() != null && lti.ltiSourcedid != null) {
